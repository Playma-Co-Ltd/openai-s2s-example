...
const service = ({logger, makeService}) => {
  ...
  const svc = makeService({path: '/openai-s2s-csv-search'});
        ...
        .llm({
          ...
          llmOptions: {
            response_create: {
              ...
              instructions: `你是專業的科技資訊助理，保持回答簡潔口語化，不超過30字。你有多種搜尋工具可以使用：

🔍 快速搜尋工具（字符串匹配）：
- search_titles：搜尋文章標題，速度最快
- search_summaries：搜尋文章摘要，速度快  
- search_tags：搜尋標籤分類，速度快
- search_hybrid：混合搜尋標題+摘要+標籤

🎯 語義搜尋工具（推薦使用）：
- search_vector：向量語義搜尋，能找到概念相關內容，速度快（500ms）

🧠 詳細搜尋工具（需要更多時間）：
- search_content：搜尋完整內容，最詳細但較慢

💡 使用策略：
1. 優先使用 search_vector 進行語義搜尋（如「語音辨識」「AI應用」）
2. 如果需要精確匹配，使用 search_titles 或 search_hybrid
3. 需要深入資訊才使用 search_content
4. 保持回答簡潔口語化，避免念稿感
5. 所有問題都要使用繁體中文回答
6. 不要念連結
7. 不要輸出任何符號
...
一開始先自我介紹，然後詢問用戶對哪方面的AI人工智慧科技資訊感興趣。`,
              voice: 'shimmer',
...
const searchOptimized = (searchType, query, maxResults = 5, extraArgs = []) => {
  return new Promise((resolve, reject) => {
    const scriptPath = path.join(__dirname, '../../csv-utils/csv_search_optimized.py');
...
const searchUltraFast = (searchType, query, maxResults = 5) => {
  return new Promise((resolve, reject) => {
    const scriptPath = path.join(__dirname, '../../csv-utils/csv_search_ultra_fast.py');
    const projectRoot = path.join(__dirname, '../..');
    const venvPython = path.join(projectRoot, '.venv/bin/python');
...
const searchVector = (searchType, query, maxResults = 5) => {
  return new Promise((resolve, reject) => {
    const scriptPath = path.join(__dirname, '../../csv-utils/csv_search_vector_fast.py');
    const projectRoot = path.join(__dirname, '../..');
    const venvPython = path.join(projectRoot, '.venv/bin/python');
...
const onToolCall = async(session, evt) => {
  ...
  try {
    if (name === 'search_titles') {
      ...
      const allowedScales = ['fahrenheit', 'celsius'];
      const unit = allowedScales.includes(String(scale)) ? scale : 'celsius';

      const encodedLocation = encodeURIComponent(location);
      let url = `https://geocoding-api.open-meteo.com/v1/search?name=${encodedLocation}&count=1&language=en&format=json`;
