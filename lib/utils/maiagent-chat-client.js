...
class MaiAgentChatClient {
  ...
  async sendMessage(sessionId, message, options = {}) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        this.logger.debug({sessionId, conversationId, message, attempt}, 'Sending message to MaiAgent using completions API');
        ...
        if (!conversationId && response.data.conversationId) {
          ...
          this.logger.info({sessionId, conversationId}, 'Extracted and stored conversation ID from response');
        }
        ...
        if (response.data && response.data.content) {
          ...
          this.logger.warn({responseData: response.data}, 'Unexpected response format from MaiAgent');
          aiResponse = '讓我想想該怎麼回應你！';
        }
        
        this.logger.info({sessionId, conversationId, response: aiResponse, attempt}, 'Received MaiAgent response');
        return aiResponse;
        ...
        if (attempt < maxRetries) {
          ...
          this.logger.info({sessionId, attempt, waitTime}, `Retrying MaiAgent API call in ${waitTime}ms`);
        ...
        const fallbackResponse = this.generateFallbackResponse(message);
        this.logger.info({sessionId, message, response: fallbackResponse}, 'Generated fallback MaiAgent response');
        return fallbackResponse;
      }
    }
  }
  ...
  generateFallbackResponse(message) {
    ...
    // 檢查是否是問候語
    if (lowerMessage.includes('你好') || lowerMessage.includes('哈囉') || lowerMessage.includes('hi') || lowerMessage.includes('hello')) {
      return '您好！我是您的技術客服語音助理。很抱歉，我現在暫時無法連接到知識庫，但我很樂意為您提供協助。請稍後再試，或者您可以重新描述您的問題。';
    }
    
    // 檢查是否是技術相關問題
    if (lowerMessage.includes('技術') || lowerMessage.includes('api') || lowerMessage.includes('程式') || lowerMessage.includes('軟體')) {
      return '很抱歉，我現在暫時無法查詢技術資料庫。請稍後再試，或者您可以提供更具體的技術問題，我會盡力協助您。';
    }
    
    // 檢查是否是日期查詢
    if (lowerMessage.includes('日期') || lowerMessage.includes('時間') || lowerMessage.includes('今天') || lowerMessage.includes('2025')) {
      return '很抱歉，我現在暫時無法查詢特定日期的資料。請稍後再試，或者您可以提供更具體的查詢內容。';
    }
    
    // 檢查是否是知識庫查詢
    if (lowerMessage.includes('知識庫') || lowerMessage.includes('資料') || lowerMessage.includes('查詢') || lowerMessage.includes('搜尋')) {
      return '很抱歉，我現在暫時無法連接到知識庫進行查詢。請稍後再試，或者您可以重新描述您想查詢的內容。';
    }
    
    // 預設回應
    return '很抱歉，我現在暫時無法提供完整的回應。請稍後再試，或者您可以重新描述您的問題。';
  }
  ...
  async sendMessageStreaming(sessionId, message, options = {}) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        this.logger.debug({sessionId, conversationId, message, attempt}, 'Sending streaming message to MaiAgent');
        ...
        return new Promise((resolve, reject) => {
          response.data.on('data', (chunk) => {
            ...
            try {
              if (chunkText.trim().startsWith('{') && chunkText.trim().endsWith('}')) {
                ...
                // 從串流回應中提取 conversation ID
                if (!conversationId && chunkData.conversationId) {
                  conversationId = chunkData.conversationId;
                  this.conversations.set(sessionId, conversationId);
                  this.logger.info({sessionId, conversationId}, 'Extracted conversation ID from streaming response');
                }
              }
            } catch (err) {
              // 部分塊，繼續累積
          ...
          response.data.on('end', () => {
            this.logger.info({sessionId, response: completeResponse, attempt}, 'Streaming response complete');
            // 嘗試解析最終的回應
            try {
          ...
          response.data.on('error', (err) => {
            this.logger.error({err, attempt}, 'Streaming error');
            reject(err);
          });
        });
        ...
        if (attempt < maxRetries) {
          const waitTime = 1000 * attempt; // 1秒, 2秒, 3秒
          this.logger.info({sessionId, attempt, waitTime}, `Retrying MaiAgent streaming API call in ${waitTime}ms`);
          await new Promise(resolve => setTimeout(resolve, waitTime));
          continue;
        }
