...
class TaskLlmMaiAgent_S2S extends Task {
  constructor(logger, opts, parentTask) {
    ...
    if (typeof response_create !== 'object') {
      throw new Error('llmOptions with an initial response.create is required for MaiAgent S2S');
    }
  ...
  async _startListening(cs, ep) {
    ...
    try {
      ...
      // 創建 MaiAgent 會話
      this.conversationId = await this.maiAgentClient.createConversation(cs.call_sid, {
        source: 'jambonz-phone-call',
        type: 'voice-call'
      });
  ...
  _populateEvents(events) {
    if (events.includes('all')) {
      const exclude = events
        ...
        .map((evt) => evt.slice(1));
      if (exclude.length === 0) this.includeEvents = maiagent_server_events;
      else this.excludeEvents = exclude;
    }
    else {
